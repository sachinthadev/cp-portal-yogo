...
window.onload = function () {
    ...
    $('#myTripsNav').addClass('active');//navigation bar active tab
    // // Load trips
    // getTripsList();
...
const getTripsList = async () => {
    ...
    try {
        let result = await fetchPost('../service/get-my-trip-list.php', {});
        stopLoading(load);
        ...
        if (result.status === 0) {
            if (result.data.status === 0) {
                ...
                const tableData = tripListData.map(trip => [
                    trip.booking_id,
                    trip.pickup_address,
                    trip.drop_address,
                    trip.employee_name,
