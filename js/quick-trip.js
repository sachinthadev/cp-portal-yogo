...
$(document).ready(function () {
  ...
  flatpickr("#tripDateTime", {
    ...
    dateFormat: "Y-m-d H:i", // only year-month-day hours:minutes
...
// --- Submit handler ---
$("#newQuickTripForm").submit(async function (event) {
...
if (requiredTimeSelect === "1") {
    // Immediate → current date & time (no seconds)
    const now = new Date();
    ...
    // Scheduled → take from Flatpickr field
    ...
    console.log("Booking Request Data:", data);
       ...
       try {
    let result = await fetchPost('../service/submit-new-quick-trip.php', data);

    if (result.status === 0) {
        const bookingId = result.data?.info?.booking_id || "N/A";  // Safe access
